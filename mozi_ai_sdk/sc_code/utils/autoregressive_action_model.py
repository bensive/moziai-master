...
class TorchAutoregressiveActionModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config,
        ...
        # by dixit
        if action_space != Tuple([Discrete(3), Discrete(2)]):
            raise ValueError(
                "This model only supports the [3, 2] action space")
    ...
    def forward(self, input_dict, state, seq_lens):
        # by dixit
        # self._context = self.context_layer(input_dict["obs"])  # ray自带
        self._context = self.context_layer(input_dict["obs_flat"])
