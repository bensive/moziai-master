...
class QMixLoss(nn.Module):
    ...
    def forward(self,
        ...
        if has_avail_actions:
            mac_out = _temp_unroll_mac(self.model, obs, avail_actions)
        else:
            mac_out = _unroll_mac(self.model, obs)
        ...
        # by dixit
        if has_avail_actions:
            target_mac_out = _temp_unroll_mac(self.target_model, next_obs, next_avail_actions)
        else:
            target_mac_out = _unroll_mac(self.target_model, next_obs)
        ...
        if self.double_q:
            ...
            # by dixit
            if has_avail_actions:
                mac_out_tp1 = _temp_unroll_mac(self.model, next_obs, next_avail_actions)
            else:
                mac_out_tp1 = _unroll_mac(self.model, next_obs)
...
class QMixTorchPolicy(Policy):
    ...
    def __init__(self, obs_space, action_space, config):
        ...
        if isinstance(agent_obs_space, Dict):
            ...
            # by dixit
            if "avail_actions" in space_keys:
                self.has_avail_actions = True
    ...
    def compute_actions(self,
        ...
        # by dixit
        obs_batch, action_mask, _, avail_actions = self._unpack_observation(obs_batch)
        ...
        with torch.no_grad():
            ...
            if self.has_avail_actions:
                q_values, hiddens = _temp_mac(
                    ...
                    torch.as_tensor(
                        avail_actions, dtype=torch.float, device=self.device)
                )
            else:
                q_values, hiddens = _mac(
                    self.model,
    ...
    def learn_on_batch(self, samples):
        ...
        # by dixit
        obs_batch, action_mask, env_global_state, avail_actions = self._unpack_observation(
            samples[SampleBatch.CUR_OBS])
        (next_obs_batch, next_action_mask,
         next_env_global_state, next_avail_actions) = self._unpack_observation(
            samples[SampleBatch.NEXT_OBS])
        ...
        # by dixit
        if self.has_avail_actions:
            input_list.extend([avail_actions, next_avail_actions])
        ...
        if self.has_env_global_state:
            ...
            if self.has_avail_actions:
                (rew, action_mask, next_action_mask, act, dones, obs,
                 next_obs, avail_actions, next_avail_actions) = output_list
            else:
                (rew, action_mask, next_action_mask, act, dones, obs,
                 next_obs) = output_list
        ...
        # by dixit
        if self.has_avail_actions:
            action_embed = self.observation_space.original_space[0]["avail_actions"].shape[1]
            avail_actions = to_batches(avail_actions, torch.float).reshape(
            [B, T, self.n_agents, self.n_actions, action_embed])
            next_avail_actions = to_batches(next_avail_actions, torch.float).reshape(
            [B, T, self.n_agents, self.n_actions, action_embed])
        ...
        loss_out, mask, masked_td_error, chosen_action_qvals, targets = (
            self.loss(rewards, actions, terminated, mask, obs, next_obs,
                      action_mask, next_action_mask, env_global_state,
                      next_env_global_state, avail_actions, next_avail_actions,
                      self.has_avail_actions))
    ...
    def _unpack_observation(self, obs_batch):
        ...
        if self.has_avail_actions:
            action_embed = self.observation_space.original_space[0]["avail_actions"].shape[1]
            avail_actions = np.concatenate(
                [o["avail_actions"] for o in unpacked], axis=1).reshape(
                [len(obs_batch), self.n_agents, self.n_actions, action_embed])
        else:
            # action_embed = self.observation_space.original_space[0]["avail_actions"].shape[1]
        ...
        # by dixit
        return obs, action_mask, state, avail_actions
...
# by dixit
def _temp_mac(model, obs, h, avail_actions):
    """Forward pass of the multi-agent controller.
    ...
    B, n_agents = obs.size(0), obs.size(1)
    if not isinstance(obs, dict):
        obs = {"obs": obs, "avail_actions": avail_actions}
    # by dixit
    # if not isinstance(avail_actions, dict):
    #     avail_actions = {"avail_actions": avail_actions}
    obs_agents_as_batches = {k: _drop_agent_dim(v) for k, v in obs.items()}
    # by dixit
    # avail_actions_agents_as_batches = {k: _drop_agent_dim(v) for k, v in avail_actions.items()}
    h_flat = [s.reshape([B * n_agents, -1]) for s in h]
    q_flat, h_flat = model(obs_agents_as_batches, h_flat, None)
    return q_flat.reshape(
        [B, n_agents, -1]), [s.reshape([B, n_agents, -1]) for s in h_flat]
...
# by dixit
def _temp_unroll_mac(model, obs_tensor, avail_actions_tensor):
    """Computes the estimated Q values for an entire trajectory batch"""
    B = obs_tensor.size(0)
    T = obs_tensor.size(1)
    n_agents = obs_tensor.size(2)
    ...
    for t in range(T):
        # by dixit
        q, h = _temp_mac(model, obs_tensor[:, t], h, avail_actions_tensor[:, t])
        mac_out.append(q)
    mac_out = torch.stack(mac_out, dim=1)  # Concat over time
