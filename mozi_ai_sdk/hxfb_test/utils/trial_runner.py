...
from ray.managers.utils import print_message
...
# ray request port
port_requestor = zmq_context.socket(zmq.REQ)
# ray release port
ray_release_port = zmq_context.socket(zmq.REQ)


def release_port(_training_id):
    try:
        ray_release_port.connect("tcp://%s:%s" % (str(BACKEND_SERVER_IP), str(BACKEND_SERVER_PORT)))
        message = {
            'zmq_command': 'train_complete',
            'training_id': _training_id
        }
        ray_release_port.send_pyobj(message)
        recv_msg = ray_release_port.recv_pyobj()
        ...
        print(Exception)
        print('fail release mozi docker!')
        sys.exit(1)


def request_port(_schedule_addr, _schedule_port, _training_id, _num_workers):
    try:
        port_requestor.connect("tcp://%s:%s" % (str(_schedule_addr), str(_schedule_port)))
        message = {}
        message['zmq_command'] = 'request_docker_port'
        message['training_id'] = _training_id
        message['num_workers'] = int(_num_workers)
        print_message(f"向总控请求docker资源, worker个数：{message['num_workers']}")
        port_requestor.send_pyobj(message)
        reply_message = port_requestor.recv_pyobj()
        if reply_message['zmq_command'] == 'request_docker_port_reply':
            pass
        else:
            sys.exit(1)
        docker_ip_port_list = reply_message['docker_ip_port_list']
        return docker_ip_port_list
    except Exception:
        print(Exception)
...
class TrialRunner:
    ...
    def __init__(self,
        ...
        self._session_str = datetime.datetime.fromtimestamp(
    ...
    def step(self):
        ...
        if next_trial is not None:
            with warn_if_slow("start_trial"):
                # by dixit 2020/12/8
                # TODO 一次性请求所有
                if 'env_config' in next_trial.config.keys() and 'mode' in next_trial.config['env_config'].keys():
                    if next_trial.config['env_config']['mode'] == 'train':
                        next_trial.config['env_config']['avail_docker_ip_port'] = []
                        next_trial.config['env_config']['avail_docker_ip_port'] = \
                            request_port(next_trial.config['env_config']['schedule_addr'],
                                        next_trial.config['env_config']['schedule_port'],
                                        next_trial.config['env_config']['training_id'],
                                        next_trial.config['num_workers'] + 1)     # +1 原因不明，勿删
                    elif next_trial.config['env_config']['mode'] == 'development':
                        # TODO
                        from ray.remote_handle_docker import generate_docker
                        sever_docker_dict = next_trial.config['env_config']['sever_docker_dict']
                        next_trial.config['env_config']['avail_docker_ip_port'] = generate_docker(sever_docker_dict)
                    else:
                        pass
    ...
    def _execute_action(self, trial, decision):
        ...
        if decision == TrialScheduler.CONTINUE:
            ...
            # by dixit
            # kill_proc()
            if 'env_config' in trial.config.keys() and 'mode' in trial.config['env_config'].keys():
                if trial.config['env_config']['mode'] == 'train':
                    release_port(trial.config['env_config']['training_id'])
                elif trial.config['env_config']['mode'] == 'development':
                    # TODO
                    for rel_ip_port in trial.config['env_config']['avail_docker_ip_port']:
                        from ray.remote_handle_docker import release_docker
                        release_docker(rel_ip_port)
                else:
                    pass
        else:
