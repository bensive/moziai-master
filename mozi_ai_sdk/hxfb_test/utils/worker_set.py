...
class WorkerSet:
    ...
    def _make_worker(
        ...
        # by dixit 2021/3/25
        if 'env_config' in self._remote_config.keys() \
                and 'env_config' in self._remote_config.keys() \
                and 'mode' in self._local_config['env_config'].keys() \
                and 'mode' in self._local_config['env_config'].keys():
            if self._remote_config['env_config']['mode'] in ['train', 'versus', 'eval', 'development'] \
                    and self._local_config['env_config']['mode'] in ['train', 'versus', 'eval', 'development']:
                if self._remote_config['env_config']['avail_docker_ip_port'] \
                        and self._local_config['env_config']['avail_docker_ip_port']:
                    self._remote_config['env_config']['avail_docker_ip_port'].pop(0)
                    self._local_config['env_config']['avail_docker_ip_port'].pop(0)
                else:
                    raise Exception('worker_set._make_worker: no avail port!')
            else:
                pass
